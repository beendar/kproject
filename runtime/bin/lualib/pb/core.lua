...
local function encode( message, t , func , ...)
	...
	if func then
		...
		local ret,retlen = func(buffer, len, ...)
...
local function decode(typename, buffer, length)
	local ret = {}
...
local function register_file(filename)
...
local function enum_id(enum_type, enum_name)
...
--for dynamic proto refreshing
local function reset()
	--do return end --currently disabled
	tclear(_pattern_cache)
	tclear(default_cache)
	tclear(encode_type_cache)
	tclear(decode_type_cache)
	P = c._env_new()
	GC = c._gc(P)
end
